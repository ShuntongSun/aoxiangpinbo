    ...
    def load_data(self):
        ...
        if self.split == 'train':
            ...
            if 'x_test' in npz_data.files:
                ...
                if 'y_test' in npz_data.files:
                    self.label = npz_data['y_test']
                else:
                    print("No labels found for test data. Using dummy labels.")
                    self.label = np.zeros(self.data.shape[0])  # 创建虚拟标签
                self.sample_name = ['test_' + str(i) for i in range(len(self.data))]
            else:
                raise ValueError("Cannot find 'x_test' in the data file.")
        else:
